...
<?php
	...
	if (function_exists('wp_list_comments')) {
		...
		$trackbacks = (isset($comments_by_type['pings']) && is_array($comments_by_type['pings'])) ? $comments_by_type['pings'] : array();
		$comment_list = (isset($comments_by_type['comment']) && is_array($comments_by_type['comment'])) ? $comments_by_type['comment'] : array();
...
<?php if (!empty($all_comments) || comments_open()) : ?>
<div id="comments">

<div id="cmtswitcher">
	<a id="commenttab" class="curtab" href="javascript:void(0);" onclick="MGJS.switchTab('thecomments,commentnavi', 'thetrackbacks', 'commenttab', 'curtab', 'trackbacktab', 'tab');"><?php _e('Comments'); echo (' (' . count($comment_list) . ')'); ?></a>
	...
	<?php if(comments_open()) : ?>
		<span class="addcomment"><a href="#respond"><?php _e('Leave a comment'); ?></a></span>
	<?php endif; ?>
	<?php if(pings_open()) : ?>
...
<div id="commentlist">
	...
	?>
<!--		<li class="messagebox"> --><?php //_e('No comments yet.', 'inove'); ?><!-- </li> -->
	<?php
		}
	?>
	</ol>
...
?>
	...
	<ol id="thetrackbacks">
		...
		<?php else : ?>
<!--			<li class="messagebox"><?php //_e('No trackbacks yet.', 'inove'); ?></li>-->
		<?php endif; ?>
	</ol>
	<div class="fixed"></div>
	<!-- trackbacks END -->
		...
		<?php if ($user_ID) : ?>
			...
			<div id="author_info">
				<div class="row">
					...
					<label for="author" class="small"><?php _e('Name', 'inove'); ?> <?php if ($req) _e('(required)'); ?></label>
				</div>
				<div class="row">
					<input type="text" name="email" id="email" class="textfield" value="<?php echo $comment_author_email; ?>" size="24" tabindex="2" />
					<label for="email" class="small"><?php _e('Email (will not be published)', 'inove');?> <?php if ($req) _e('(required)'); ?></label>
				</div>
				<div class="row">
